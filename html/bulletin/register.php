...
<!-- 今のところ、フォームを送信されていない場合とフォームは送信されたけど入力内容に誤りがある場合は、form_register.phpを表示する。 -->
<!-- フォームが送信されて入力内容に誤りがない場合は確認ページを表示する。 -->
<!-- 仮登録が完了したか指定なかでアクションを変える。 -->
<?php $_action = (isset($pre_registered)) ? 'register.php' : 'pre_register.php' ?>

<div id="contents">
    <h2>会員登録</h2>
    <?php if (empty($errors) && $do_confirm) : ?>
        <div class="confirm">
            <table>
                <tbody>
                    <tr>
            ...
            <form class="actionForm" action="register.php" method="post">
    ...
    <?php elseif (empty($errors) && $do_register) : ?>
        <div class="comment">
    ...
    <?php else : ?>
        ...
        <form class="default" action="<?php echo get_uri('pre_register.php') ?>" method="post">
            <div class="item">
                <p class="title">名前</p>
                <p class="input"><input type="text" name="name" value="<?php echo h($name) ?>" /></p>
            </div>
            <div class="item">
                <p class="title">メール</p>
